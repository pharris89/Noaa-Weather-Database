# -*- coding: utf-8 -*-
"""
Created on Sun Apr 10 15:50:34 2022

@author: pharr
"""

#Purpose: Create Celsius plot comparing period 1 and period 2
#Name: Paula Harris
#Date: 04/10/2022
import pandas as pd
import matplotlib.pyplot as plt
df1 = pd.read_csv("formatdata.csv") #baseline data is period 1 (older)
df2 = pd.read_csv("formatdata2.csv") #data for period 2 (more recent)
plt.figure(); df1.Fahrenheit.plot(label = 'period 1'); df2.Fahrenheit.plot(label = 'period 2'); plt.legend(loc='best'); plt.suptitle('Fahrenheit')
plt.show()
